...
class CCRS2ProductionWorker:
    def __init__(self):
        """Initialize worker with Redis connection and Claude CLI verification"""
        # Check Claude CLI availability
        if not shutil.which("claude"):
            raise RuntimeError("Claude CLI not found in PATH. Please install Claude Code CLI first.")

        print(f"✅ Claude CLI found: {shutil.which('claude')}")
    ...
    def execute_claude_command(self, operation: str, message: str, timeout: int = 300) -> Dict[str, Any]:
        """Execute real Claude CLI command and return result"""
        try:
            if operation == "chat":
                # Claude chat command
    ...
    def run(self):
        """Main worker loop"""
        print("🚀 CCRS-2 Production Worker starting...")
        print("🎯 Using real Claude CLI integration")
